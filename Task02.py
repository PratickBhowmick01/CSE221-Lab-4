...
def dijkstra(graph, source, dest):
    #list for distance, visited and prev
    dist, prev, vis = [1000]*(dest+1), [None]*(dest+1), [False]*(dest+1) 
    dist[source] = 0   
    #Priority queue
    q = []
...
for i in range(t):        #loop for test cases
    ...
    elif(m == 1):       #only one road available
        line = f.readline()
    ...
    else:       #need to find shortest path using dijkstra 
        graph = {}
        for k in range(m):
            input2 = f.readline()
